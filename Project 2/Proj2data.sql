...
/* trigger fired to delete the corresponding student from enrollment and tas table before deleting a student from the student table*/
...
/*trigger fired to decrease the class size by 1 when student is unenrolled */
...
/*trigger fired to increase the class size by 1 when student is enrolled */
...
/* procedures to display all the tables*/
...
/* to show the first name,last name and B# of TAs*/
...
begin
/* checking conditions if the class id valid or no and if it has a Ta*/
...
/* Delete a aprticular student*/
procedure delete_stud(given_B# in Students.B#%type) is
student_count number;
bn Exception;
begin
/* checking if the given B# is valid or no*/
...
/* to display the pre requisites of a particular subject */
procedure p_prerequisites(p_dept_code in prerequisites.dept_code%type, p_course# in prerequisites.course#%type, p_cursor out sys_refcursor ) is
   ...
   count_prereq number;
   exp Exception;
   /* creating a temporary table in order to store the direct and indirect pre requisites */
   begin
...
/* Procedure to enroll the student */
...
req_p number;
	...
	begin
                 /* Checking if the given B# and classid is valid or no */
		...
		/* Checking the remaining condition such as limit of the class size, if the enrollment is in Fall, if the student is already enrolled */
...
/* Procedure to drop a particular student*/
...
BEGIN
/* Conditions to check if the B# and classid is valid, if student is registered and if the student is registered has he/she registered in Fall 2018*/
